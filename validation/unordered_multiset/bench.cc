/*
  Driver to test the transactional version of the std::unordered_multiset
  interface

  According to
  http://www.cplusplus.com/reference/unordered_set/unordered_multiset/,
  the std::unordered_multiset interface consists of the following:

  Steps:
    1 - Put TBD traces into all of GCC's unordered_multiset functions
...
|                   |                  |                        Expected | Actual |
|-------------------+------------------+---------------------------------+--------|
| Member            | (constructor)    | 1a, 1b, 1c, 1d, 1e, 2a, 2b, 2c, |        |
| Functions         |                  |      3a, 3b, 4a, 4b, 5a, 5b, 5c |        |
|                   | (destructor)     |                               1 |        |
|                   | operator=        |                         1, 2, 3 |        |
|-------------------+------------------+---------------------------------+--------|
| Iterators         | begin            |                  1a, 1b, 2a, 2b |        |
|                   | end              |                  1a, 1b, 2a, 2b |        |
|                   | cbegin           |                            1, 2 |        |
|                   | cend             |                            1, 2 |        |
|-------------------+------------------+---------------------------------+--------|
...
|                   | bucket           |                               1 |        |
|-------------------+------------------+---------------------------------+--------|
| Hash Policy       | load_factor      |                               1 |        |
|                   | max_load_factor  |                            1, 2 |        |
|                   | rehash           |                               1 |        |
|                   | reserve          |                               1 |        |
|-------------------+------------------+---------------------------------+--------|
| Observers         | hash_function    |                               1 |        |
|                   | key_eq           |                               1 |        |
|                   | get_allocator    |                               1 |        |
|-------------------+------------------+---------------------------------+--------|
| Non-member        | '=='             |                               1 |        |
| function          | '!='             |                               1 |        |
| overloads (NMFOs) | swap             |                               1 |        |
|-------------------+------------------+---------------------------------+--------|
*/
